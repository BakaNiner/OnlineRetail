...
def commentGate():
    ...
    if not comments:
        ...
        for userName, grade, comment, reviewTime in comments:
            ...
            html += "<p>" + "★" * grade + "☆" * (5 - grade) + " " + str(reviewTime) + "</p>"
...
def orderGate():
    ...
    for orderID, shippingAddressID, totalAmount, orderDate, is_complete in orders:
		...
		'''.format(streetAddress, cityName, provinceName, postalCode, phoneNumber, totalAmount, orderDate,
                   "未完成" if is_complete == '0' else "已完成")

...
CREATE TABLE Product(
    ...
    CHECK((productFigure IS NULL) OR (productFigure LIKE 'static/images/%.jpg')),
) ;
...
CREATE INDEX i1_address ON ShipAddress(customerID);
...
CREATE TABLE OrderTable(
    ...
    FOREIGN KEY(shippingAddressID) REFERENCES ShipAddress(addressID),
    FOREIGN KEY(customerID) REFERENCES Customer(customerID),
    CHECK ((is_complete IS NULL) OR (is_complete = '0') OR (is_complete = '1')),
) ;

CREATE INDEX i1_orderT ON OrderTable(customerID);
...
CREATE INDEX i1_review ON Review(reviewerID);
CREATE INDEX i2_review ON Review(productID);
...
CREATE TRIGGER stock_check
    ...
    BEGIN
        ...
        DECLARE @qty INT
        DECLARE @inv INT
        SELECT @orderid=orderID, @id = itemID, @qty = quantity FROM INSERTED
        SELECT @inv = stockAmount FROM Product WHERE productID=@id
        IF @inv < @qty
            UPDATE OrderITEM SET quantity = @inv WHERE orderID=@orderid
    end
